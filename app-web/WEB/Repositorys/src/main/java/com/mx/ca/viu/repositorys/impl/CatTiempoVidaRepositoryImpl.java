 ...
 */
@Service(value = "catTiempoVidaRepository")
public class CatTiempoVidaRepositoryImpl extends SimpleRepository implements CatTiempoVidaRepository{
    private static final Logger logger = LogManager.getLogger(CatTiempoVidaRepositoryImpl.class.getName());
    ...
    public List<CatValoresTiempoVida> buscarTodos(boolean activos) {
        ...
        try {
            if (activos) {
                respuesta = (List<CatValoresTiempoVida>) getSession().createQuery("select r from CatValoresTiempoVida r where r.status=true ORDER BY r.nombre").list();

            } else {
                respuesta = (List<CatValoresTiempoVida>) getSession().createQuery("select r from CatValoresTiempoVida r ORDER BY r.nombre").list();
    ...
    public List<CatValoresTiempoVida> buscarTodosXEmpresa(boolean activos, Long IdEmpresa) {
        ...
        try {
            if(activos){
                respuesta = (List<CatValoresTiempoVida>) getSession().createQuery("select r from CatValoresTiempoVida r where r.status=true and r.idEmpresa.idEmpresas in (1,:empresa) ORDER BY r.idEmpresa.idEmpresas DESC")
                        .setParameter("empresa", IdEmpresa).list();
            }else{
                respuesta = (List<CatValoresTiempoVida>) getSession().createQuery("select r from CatValoresTiempoVida r where r.idEmpresa.idEmpresas in (1,:empresa) ORDER BY r.idEmpresa.idEmpresas DESC")
                        .setParameter("empresa", IdEmpresa).list();
            }
